...
class MotorController(object):
        ...
        if(self.last_position != position): # Check if position is different from the last recorded position
            if(self.last_position != 0):
                ...
                if(self.position_counter == 30):
                    ...
                    self._calculate_rms(self.last_current_index, (len(self.data[4]) - 1))
                    self.last_current_index = (len(self.data[4]) - 1)
                    print(self.csv_data)
    ...
    def _calculate_rms(self, c_start, c_finish):
        ...
        for i in range(4, 7):
            ...
            temp_rms = temp_sum/(self.data[0][c_finish] - self.data[0][c_start - 1])
